CONTEUDO_EDUCACIONAL = {
    "Adicao": {
        "titulo": "Adição (Soma)",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A adição é a operação matemática fundamental que representa a ideia de **juntar** ou **somar** quantidades. O símbolo usado para a adição é o sinal de mais **(+)**. O resultado de uma adição é chamado de **soma** ou **total**."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Adição adequadamente:"
            },
            {
                "tipo": "lista_ordenada",
                "itens": [
                    "**Organize os números:** Para somar números com mais de um dígito, organize-os em colunas, alinhando as casas decimais (ou as unidades, de trás para frente).",
                    "**Comece pela direita:** Sempre inicie a soma pela coluna da direita (a das unidades) e vá movendo para a esquerda.",
                    "**Realize a soma e a 'subida':** Some os números de cada coluna. Se a soma for igual ou maior que 10, você deixa o algarismo das unidades e 'sobe' o algarismo das dezenas para a próxima coluna.",
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "Exemplo: para somar 25 + 18"
            },
            {
                "tipo": "codigo",
                "conteudo": "  25  (5 + 8 = 13, 3 fica na coluna das unidades e 1 vai pra casa das dezenas)\n+ 18  (2 + 1 + 1 = 4)\n-----\n  43"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A adição é a base para a maioria dos cálculos e é a operação mais simples de entender, sendo usada para combinar valores, contar objetos ou calcular totais."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é 12 + 5?", "resposta": "17"},
            {"pergunta": "Quanto é 30 + 15?", "resposta": "45"},
            {"pergunta": "Quanto é 47 + 28?", "resposta": "75"},
            {"pergunta": "Quanto é 99 + 1?", "resposta": "100"},
            {"pergunta": "Quanto é 36 + 44?", "resposta": "80"},
            {"pergunta": "Quanto é 46 + 27?", "resposta": "73"},
            {"pergunta": "Quanto é 73 + 19?", "resposta": "92"},
            {"pergunta": "Quanto é 32 + 16?", "resposta": "48"},
            {"pergunta": "Quanto é 45 + 19?", "resposta": "64"},
            {"pergunta": "Quanto é 67 + 12?", "resposta": "79"}
        ]
    },
    "Subtracao": {
        "titulo": "Subtração",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A subtração é a operação matemática que representa a ideia de **diminuir**, **tirar** ou encontrar a **diferença** entre duas quantidades. Ela é a operação inversa da adição. O símbolo usado é o sinal de menos **(-)**."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Subtração adequadamente:"
            },
            {
                "tipo": "lista_ordenada",
                "itens": [
                    "**Organize os números:** Assim como na adição, alinhe os números em colunas, com o maior valor (o **minuendo**) em cima e o menor (o **subtraendo**) embaixo.",
                    "**Comece pela direita:** Inicie a subtração pela coluna das unidades (da direita) e avance para a esquerda.",
                    "**Realize a subtração:** Subtraia o número de baixo do número de cima em cada coluna.",
                    "**Subtração com 'empréstimo' (quando o de cima é menor): Subtraia 1 do minuendo à esquerda para adicioná-lo ao minuendo que está sendo calculado.**"
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "Exemplo: para subtrair 62 - 35"
            },
            {
                "tipo": "codigo",
                "conteudo": "  62  /  (2 - 5 resulta em negativo, mas com o 'empréstimo' fazemos 12 - 5 = 7)\n- 35  /  (6 - 3 passou por uma redução no valor de cima, virando 5 - 3 = 2)\n-----\n  27"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A subtração é essencial para calcular trocos, encontrar diferenças de idade ou de valores, e resolver uma variedade de problemas cotidianos."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é 10 - 3?", "resposta": "7"},
            {"pergunta": "Quanto é 25 - 10?", "resposta": "15"},
            {"pergunta": "Quanto é 50 - 22?", "resposta": "28"},
            {"pergunta": "Quanto é 80 - 45?", "resposta": "35"},
            {"pergunta": "Quanto é 100 - 58?", "resposta": "42"},
            {"pergunta": "Quanto é 64 - 29?", "resposta": "35"},
            {"pergunta": "Quanto é 91 - 34?", "resposta": "57"},
            {"pergunta": "Quanto é 73 - 18?", "resposta": "55"},
            {"pergunta": "Quanto é 88 - 47?", "resposta": "41"},
            {"pergunta": "Quanto é 55 - 27?", "resposta": "28"}
        ]
    },
    "Multiplicacao": {
        "titulo": "Multiplicação",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A multiplicação é uma operação matemática que representa uma **soma repetida**. Em vez de somar o mesmo número várias vezes, você usa a multiplicação para chegar ao resultado de forma mais rápida. O sinal usado para a multiplicação é o '×' ou, mais frequentemente, o ponto '•'.\nOs números que você está multiplicando são chamados de **fatores**, e o resultado é o **produto**."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Multiplicação adequadamente:"
            },
            {
                "tipo": "lista_ordenada",
                "itens": [
                    "**Organize os números:** Alinhe os números em colunas. Se você está multiplicando por um número de apenas um dígito, o processo é mais direto.",
                    "**Multiplique por cada dígito:** Comece multiplicando o número de baixo por cada dígito do número de cima, da direita para a esquerda.",
                    "**Realize a multiplicação e a 'subida':** Caso o produto de uma coluna seja 10 ou mais, escreva o algarismo das unidades e 'suba' o algarismo das dezenas para a próxima coluna."
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "Exemplo: Para multiplicar 34 × 2:"
            },
            {
                "tipo": "codigo",
                "conteudo": "  34  (4 × 2 = 4 + 4 = 8)\n×  2  (3 × 2 = 3 + 3 = 6\n-----\n  68"
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Multiplicação por um número com mais de um dígito:"
            },
            {
                "tipo": "lista_ordenada",
                "itens": [
                    "**Multiplique pelo primeiro dígito de baixo:** usaremos 24 × 12 como exemplo\nPrimeiro, multiplique 34 por 2. O resultado é 68.",
                    "**Multiplique pelo segundo dígito de baixo:** Multiplique 34 por 1, mas como o 1 representa 10, adicione um zero (0) na coluna das unidades para alinhar corretamente. O resultado é 340.",
                    "**Adicione os resultados parciais:** Some os dois resultados que você encontrou: 68 e 340. O resultado final é 408."
                ]
            },
            {
                "tipo": "codigo",
                "conteudo": "  34  (2 × 4 = 8 e 2 × 3 = 6, resultado 68)\nx 12  (1 × 4 = 4 e 1 × 3 = 3, resultado 34, que, adicionado o 0, vira 340)\n-----\n  68  (34 × 2)\n+340  (34 × 10)\n-----\n 408"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A multiplicação é uma ferramenta poderosa para calcular áreas, valores totais e para resolver problemas que envolvem repetição, tornando-a uma das operações mais versáteis da matemática."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é 6 × 7?", "resposta": "42"},
            {"pergunta": "Quanto é 15 × 3?", "resposta": "45"},
            {"pergunta": "Quanto é 9 × 8?", "resposta": "72"},
            {"pergunta": "Quanto é 12 × 4?", "resposta": "48"},
            {"pergunta": "Quanto é 11 × 11?", "resposta": "121"},
            {"pergunta": "Quanto é 14 × 5?", "resposta": "70"},
            {"pergunta": "Quanto é 20 × 6?", "resposta": "120"},
            {"pergunta": "Quanto é 7 × 9?", "resposta": "63"},
            {"pergunta": "Quanto é 13 × 7?", "resposta": "91"},
            {"pergunta": "Quanto é 8 × 12?", "resposta": "96"}
        ]
    },
    "Divisao": {
        "titulo": "Divisão",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A divisão é a operação matemática que representa a ideia de **repartir** uma quantidade em partes iguais. Ela é a operação inversa da multiplicação. O símbolo usado pode ser o sinal de divisão (÷) ou dois pontos (:).\nO número que está sendo dividido é o **dividendo**, o número pelo qual você divide é o **divisor**, o resultado é o **quociente**, e o que sobra é o **resto**."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Divisão adequadamente:"
            },
            {
                "tipo": "lista_ordenada",
                "itens": [
                    "**Arme a conta:** Coloque o dividendo dentro de uma 'chave' e o divisor fora.",
                    "**Divida o primeiro algarismo:** Comece pelo primeiro algarismo da esquerda do dividendo e repita o processo de dividir, multiplicar e subtrair.",
                    "**Abaixe o próximo algarismo:** Abaixe o próximo algarismo do dividendo ao lado do resto."
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "Exemplo: Para dividir 48 ÷ 4:"
            },
            {
                "tipo": "codigo",
                "conteudo": "  48        | 4\n- 4 (4 × 1)  ---\n ---          12\n  08\n-  8(4 × 2)\n ---\n   0"
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Divisão com 'empréstimo' e resto:"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "**Exemplo:** Ao dividir 97 ÷ 4, o resultado é 24, e o resto é 1. Se você quiser continuar a conta para obter um resultado decimal, você adiciona uma vírgula no quociente e um zero no resto, repetindo o processo."
            },
            {
                "tipo": "codigo",
                "conteudo": "  97        | 4\n- 8 (4 × 2)  ---\n ---          24\n  17\n- 16(4 × 4)\n ---\n   1"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A divisão é fundamental para resolver problemas de distribuição, calcular médias e encontrar a quantidade de grupos que podem ser formados."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é 20 ÷ 4?", "resposta": "5"},
            {"pergunta": "Quanto é 45 ÷ 5?", "resposta": "9"},
            {"pergunta": "Quanto é 81 ÷ 9?", "resposta": "9"},
            {"pergunta": "Quanto é 100 ÷ 10?", "resposta": "10"},
            {"pergunta": "Quanto é 56 ÷ 7?", "resposta": "8"},
            {"pergunta": "Quanto é 72 ÷ 8?", "resposta": "9"},
            {"pergunta": "Quanto é 90 ÷ 3?", "resposta": "30"},
            {"pergunta": "Quanto é 64 ÷ 4?", "resposta": "16"},
            {"pergunta": "Quanto é 49 ÷ 7?", "resposta": "7"},
            {"pergunta": "Quanto é 36 ÷ 6?", "resposta": "6"}
        ]
    },
    "Potenciacao": {
        "titulo": "Potenciação",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A potenciação é a operação matemática que representa uma **multiplicação de fatores iguais**. Em vez de escrever 2×2×2×2, usamos a potenciação para simplificar a notação.\nEla é composta por três elementos: **Base** (o número que se repete), **Expoente** (o número de vezes que a base se repete) e **Potência** (o resultado).\n"
                r"A forma geral é: aⁿ, onde 'a' é a base e 'n' é o expoente."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Potenciação adequadamente:"
            },
            {
                "tipo": "lista_ordenada",
                "itens": [
                    "**Entenda o expoente:** O expoente não é para multiplicar, mas sim para indicar quantas vezes a base é multiplicada por ela mesma.",
                    r"**Realize a multiplicação:** por exemplo, $5^3$ significa $5 \times 5 \times 5 = 125$."
                ]
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Regras importantes da Potenciação:"
            },
            {
                "tipo": "lista_nao_ordenada",
                "itens": [
                    r"**Expoente 1:** Qualquer número elevado a 1 é igual a ele mesmo (ex: $7^1 = 7$).",
                    r"**Expoente 0:** Qualquer número (exceto o zero) elevado a 0 é igual a 1 (ex: $10^0 = 1$).",
                    r"**Base 10:** Quando a base é 10, o resultado é o número 1 seguido de tantos zeros quanto o valor do expoente (ex: $10^4 = 10.000$).",
                    r"**Números negativos:** A base negativa com expoente par resulta em n_mero positivo (ex: $(-2)^4 = 16$), e com expoente _mpar, em negativo (ex: $(-2)^3 = -8$)."
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A potenciação é fundamental para o estudo de áreas, volumes, notação científica e juros compostos, sendo uma ferramenta poderosa para simplificar cálculos com números muito grandes ou muito pequenos."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é 3³?", "resposta": "27"},
            {"pergunta": "Quanto é 2⁵?", "resposta": "32"},
            {"pergunta": "Quanto é 5⁴?", "resposta": "625"},
            {"pergunta": "Quanto é 10²?", "resposta": "100"},
            {"pergunta": "Quanto é 4³?", "resposta": "64"},
            {"pergunta": "Quanto é 6²?", "resposta": "36"},
            {"pergunta": "Quanto é 7³?", "resposta": "343"},
            {"pergunta": "Quanto é 8²?", "resposta": "64"},
            {"pergunta": "Quanto é 9³?", "resposta": "729"},
            {"pergunta": "Quanto é 2¹⁰?", "resposta": "1024"}
        ]
    },
    "Radiciacao": {
        "titulo": "Radiciação (Raiz)",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A radiciação é a operação matemática **inversa da potenciação**. Ela busca a **base** que, multiplicada por si mesma, resulta no número que está dentro da raiz.\nEla é composta por **índice**, **radical** (√) e **radicando**."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Radiciação adequadamente:"
            },
            {
                "tipo": "lista_nao_ordenada",
                "itens": [
                    "**Pense na potenciação:** A pergunta é 'Qual número, elevado ao índice, resulta no radicando?'",
                    r"**Exemplo:** √25. O índice é 2. A resposta é 5, pois $5^2 = 25$.",
                    r"**Exemplo:** ∛8. O índice é 3. A resposta é 2, pois $2^3 = 8$."
                ]
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Regras importantes da Radiciação:"
            },
            {
                "tipo": "lista_nao_ordenada",
                "itens": [
                    "**Raiz de números negativos:** Em geral, não existe raiz quadrada de números negativos no conjunto dos números reais.",
                    "**Simplificação de radicais:** Você pode fatorar o radicando para simplificar a raiz.",
                    r"**Exemplo:** ao fatorar √12, obtemos √2 × 2 × 3 = √$2^2$ + 3.""\nO expoente 2 pode 'cortar' com o índice 2 e o 2 sai pra fora\nO resultado é 2√3."
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A radiciação é essencial para calcular lados de triângulos usando o Teorema de Pitágoras e para resolver equações de segundo grau."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é √49?", "resposta": "7"},
            {"pergunta": "Quanto é ³√27?", "resposta": "3"},
            {"pergunta": "Quanto é √64?", "resposta": "8"},
            {"pergunta": "Quanto é ³√8?", "resposta": "2"},
            {"pergunta": "Quanto é √100?", "resposta": "10"},
            {"pergunta": "Quanto é ⁴√16?", "resposta": "2"},
            {"pergunta": "Quanto é √81?", "resposta": "9"},
            {"pergunta": "Quanto é ³√1?", "resposta": "1"},
            {"pergunta": "Quanto é √36?", "resposta": "6"},
            {"pergunta": "Quanto é ⁵√32?", "resposta": "2"}
        ]
    },
    "Logaritmicao": {
        "titulo": "Logaritmação (Logaritmo)",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "A logaritmação é a operação **inversa da potenciação** que busca o **expoente**. A pergunta que ela responde é: 'Qual expoente devo usar na base para obter este resultado?\nEla é composta por **base** (número que é elevado ao expoente, na parte de baixo do log), **logaritmando**(O resultado da potenciação, ao lado do log) e **logaritmo** (o resultado).\nA forma geral é log₁b = x, onde '1' é a base, 'b' é o logaritmando' e 'x' é o logaritmo sendo lido como 'o logaritmo de b na base 1 é igual a x'."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Como realizar a Logaritmação adequadamente:"
            },
            {
                "tipo": "lista_nao_ordenada",
                "itens": [
                    "**Transforme em potenciação:** Use a 'regra da voltinha': a base eleva o resultado do log para dar o logaritmando.",
                    "**Exemplo:** log₂8 = n. Isso se traduz para a equação 2ⁿ = 8. A resposta é 3."
                ]
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Regras importantes da Logaritmação:"
            },
            {
                "tipo": "lista_nao_ordenada",
                "itens": [
                    "**Logaritmo da base:** Se o logaritmando for igual à base, o resultado é 1 (ex: log₅5 = 1).",
                    "**Logaritmo de 1:** logaritmo de 1 em qualquer base (positiva e diferente de 1) é 0 (ex: log₉1 = 0.",
                    "**Logaritmo de 0 e negativos:** Não é definido no conjunto dos números reais."
                ]
            },
            {
                "tipo": "paragrafo",
                "conteudo": "A logaritmação é essencial para calcular escalas de medida como a escala Richter e o pH, além de ser a base para o estudo de juros compostos."
            }
        ],
        "exercicios": [
            {"pergunta": "Quanto é log₂8?", "resposta": "3"},
            {"pergunta": "Quanto é log₁₀100?", "resposta": "2"},
            {"pergunta": "Quanto é log₃27?", "resposta": "3"},
            {"pergunta": "Quanto é log₅625?", "resposta": "4"},
            {"pergunta": "Quanto é log₄256?", "resposta": "4"},
            {"pergunta": "Quanto é log₂32?", "resposta": "5"},
            {"pergunta": "Quanto é log₁₀1000?", "resposta": "3"},
            {"pergunta": "Quanto é log₆36?", "resposta": "2"},
            {"pergunta": "Quanto é log₇7?", "resposta": "1"},
            {"pergunta": "Quanto é log₈64?", "resposta": "2"}
        ]
    },
    "Fracao": {
        "titulo": "O que são Frações?",
        "teoria": [
            {
                "tipo": "paragrafo",
                "conteudo": "Uma fração representa uma **parte de um todo**. Imagine uma pizza dividida em 8 fatias iguais. Se você comer 1 fatia, você comeu 1/8 da pizza."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Componentes de uma Fração"
            },
            {
                "tipo": "codigo",
                "conteudo": "    1   -> Numerador (quantas partes você tem)\n   ---  -> Traço de fração\n    8   -> Denominador (em quantas partes o todo foi dividido)"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "O **numerador** fica em cima e diz com quantas partes do todo estamos lidando. O **denominador** fica embaixo e diz em quantas partes iguais o todo foi dividido."
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Tipos de Frações"
            },
            {
                "tipo": "lista_nao_ordenada",
                "itens": [
                    "**Fração Própria:** O numerador é **menor** que o denominador (ex: 2/5). Representa uma quantidade menor que um inteiro.",
                    "**Fração Imprópria:** O numerador é **maior** ou igual ao denominador (ex: 7/5). Representa uma quantidade igual ou maior que um inteiro.",
                    "**Fração Aparente:** É um tipo de fração imprópria onde o numerador é um múltiplo do denominador (ex: 10/5). Ela 'aparenta' ser uma fração, mas na verdade representa um número inteiro (10 ÷ 5 = 2).",
                    "**Número Misto:** A representação de uma fração imprópria como um número inteiro e uma fração própria juntos (ex: 1 2/5, que é o mesmo que 7/5)."
                ]
            },
            {
                "tipo": "subtitulo",
                "conteudo": "Frações Equivalentes e Simplificação"
            },
            {
                "tipo": "paragrafo",
                "conteudo": "**Frações equivalentes** são frações que representam a mesma quantidade, mas são escritas com números diferentes. Por exemplo, 1/2 e 2/4 são equivalentes. Você pode encontrar uma fração equivalente multiplicando ou dividindo o numerador e o denominador pelo mesmo número."
            },
            {
                "tipo": "paragrafo",
                "conteudo": "**Simplificar uma fração** significa encontrar a fração equivalente mais simples possível (irredutível), dividindo o numerador e o denominador pelo maior divisor comum entre eles."
            },
            {
                "tipo": "codigo",
                "conteudo": "Exemplo de Simplificação:\nFração: 4/8\n\nDividindo ambos por 4:\n4 ÷ 4 = 1\n8 ÷ 4 = 2\n\nFração simplificada: 1/2"
            }
        ],
        "exercicios": [
            {"pergunta": "Na fração 3/7, qual número é o denominador?", "resposta": "7"},
            {"pergunta": "Qual o nome do número que fica na parte de CIMA de uma fração?", "resposta": "numerador"},
            {"pergunta": "Como se lê a fração 2/5?", "resposta": "dois quintos"},
            {"pergunta": "A fração 5/9 é própria ou imprópria?", "resposta": "propria"},
            {"pergunta": "A fração 10/3 é própria ou imprópria?", "resposta": "impropria"},
            {"pergunta": "Qual fração é um exemplo de fração aparente? (ex: 6/3)", "resposta": "6/3"},
            {"pergunta": "Qual é a forma simplificada da fração 5/10?", "resposta": "1/2"},
            {"pergunta": "O número misto 1 1/3 é igual a qual fração imprópria?", "resposta": "4/3"},
            {"pergunta": "A fração 9/9 representa qual número inteiro?", "resposta": "1"},
            {"pergunta": "Qual o denominador na fração 12/15?", "resposta": "15"}
        ]
    }
}